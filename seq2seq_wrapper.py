...
class Seq2Seq(object):
    ...
    def __init__(self, xseq_len, yseq_len, 
        ...
        self.embedding_state.append(tf.Variable(tf.random_normal([])))
        ...
        def __graph__():
            ...
            # now, for training,
            #  build loss function
            #OK AICI SALVAM EMBEDDED STATE
#            for var in tf.trainable_variables():
#                    if var.name == 'decoder/embedding_rnn_seq2seq/RNN/EmbeddingWrapper/embedding:0':
#                       self.embedding_state = var
#                       print ("CAUTACAUTAVUAADDASDASDASDASDASDASMNDKJANSJ")
            # weighted loss
...
#IAR AICI CITEAM VALOAREA SALVATA        reader = pywrap_tensorflow.NewCheckpointReader("./OpenSubtitles/ckpt/seq2seq_modelembedded.ckpt-99000")
#        var_to_shape_map = reader.get_variable_to_shape_map()
#        for key in var_to_shape_map:
#            if key == "embedded":
#                self.embedding_state = reader.get_tensor(key)
#                print ("CAT E ? ",self.embedding_state)


        saver = tf.train.Saver()
#        salvati = tf.train.Saver({"embedded":self.embedding_state})
...
#SALVAM EMBEDDING            embedding = tf.get_variable('embedding_rnn_seq2seq/RNN/EmbeddingWrapper/embedding', [60002, 1024])
#            for var in tf.trainable_variables():
#                if var.name == 'decoder/embedding_rnn_seq2seq/RNN/EmbeddingWrapper/embedding:0':
                    #self.embedding_state = var
                    #print("ACUM ",sess.run(embedding))
                    #var = var + self.embedding_state
#SI IL ADAUGAM LA STAREA URMATOARE                    sess.run(embedding.assign(self.embedding_state +sess.run(var)))
                    #print("AVEM ", sess.run(var))
        ...
        for i in range(self.epochs):
            try:
#                with tf.variable_scope("decoder",reuse=True):
#                     embedding = tf.get_variable('embedding_rnn_seq2seq/RNN/EmbeddingWrapper/embedding', [60002, 1024])
#                     for var in tf.trainable_variables():
#                         if var.name == 'decoder/embedding_rnn_seq2seq/RNN/EmbeddingWrapper/embedding:0':
                             #self.embedding_state = var
#                print("ACUM ",sess.run(embedding))
                             #var = var + self.embedding_state
#                             sess.run(embedding.assign(self.embedding_state +sess.run(var)))
#                print("AVEM ", sess.run(var))

#                     for var in tf.trainable_variables():
#                         if var.name == 'decoder/embedding_rnn_seq2seq/RNN/EmbeddingWrapper/embedding:0':
#                            #self.embedding_state = var
#                             print("ACUM2 ",sess.run(var))
#                            self.embedding_state = sess.run(var) + self.embedding_state
#                            print ("AVEM2 ",self.embedding_state)
...
#                   salvati.save(sess, self.ckpt_path + self.model_name + 'embedded.ckpt', global_step=i)
                    # evaluate to get validation loss
                    val_loss = self.eval_batches(sess, valid_set, 16) # TODO : and this
                    # print stats
                    sys.stdout.write('\nModel saved to disk at iteration #{}'.format(i))
                    sys.stdout.write('val   loss : {0:.6f}'.format(val_loss))
                    sys.stdout.flush()
    ...
    def predict(self, sess, X):
        ...
        #  (interchange batch_size and timesteps dimensions
        dec_op_v = np.array(dec_op_v).transpose([1,0,2])
        #print(len(np.argmax(dec_op_v, axis=2)))
        #print (np.argmax(dec_op_v, axis=2))
