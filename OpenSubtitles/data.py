...
FILENAME = 'output_concatFile.txt'
import pickle
limit = {
        'maxq' : 20,
        'minq' : 0,
...
def process_data():
    ...
    w = csv.writer(open("metadata.csv","w"))
...
def load_data(PATH=''):
    ...
    metadata = {}
    #for key, val in csv.reader(open(PATH+"metadata.csv")):
    #    metadata[key] = val
    for key, val in csv.reader(open(PATH+"metadata.csv")):
        val = val.replace("{","")
        val = val.replace("}","")
        ...
        if key == 'w2idx':
            ...
            for l in val:
                ...
                vals.append(l[0])
                nums.append(l[1])
            metadata['w2idx'] = dict([(v,n) for v,n in zip(vals,nums)] )
        elif key == 'idx2w':
            val=val.split(',')
            idx2w_list = []
            for l in val:
            ...
            metadata['limit'] = dict([(l[0],l[1])])
             ...
             for l in val:
                 ...
                 l[0]=l[0].replace("'","")
                 l[1] = int(l[1])
                 freq_list.append((l[0],l[1]))
             metadata['freq_dist'] = freq_list
    # read numpy arrays
